...
## Known limitation: do not manually set `type` / `role` on generated request objects

The REST client classes (e.g. `ChatCompletionRequestMessageContentPartText`, `ChatCompletionRequestUserMessage`) are generated by swagger-codegen from an OpenAPI spec that models polymorphic fields (`type`, `role`) using `oneOf` + `discriminator`. The generated Java interfaces (`ChatCompletionRequestUserMessageContentPart`, `ChatCompletionRequestMessage`) already inject those fields into the JSON via `@JsonTypeInfo` during serialization.

**Do not call `setType(...)` / set `.role` (or similar discriminator fields) manually on these objects.** Doing so produces a duplicate JSON key (e.g. `{"type":"text","type":"text",...}`), which OpenAI's API rejects with `400 invalid_json: duplicate JSON key`. This affects any message/content-part type modeled with `oneOf` + `discriminator` in the spec, not just `text`/`user`. See the commented-out lines in `processes/openai.p.json` for reference.
